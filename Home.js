import { Text, View, TouchableOpacity, Animated, Image, PanResponder, TouchableWithoutFeedback } from 'react-native';
...
const Home = ({ navigation, route }) => {
  ...
  const VideoPressed = async () => {
    if (videoRef.current) {
      const timestamp = await videoRef.current.getStatusAsync();
      console.log(timestamp.positionMillis);
    };
  };

  return (
    <TouchableWithoutFeedback onPress={VideoPressed} {...panResponder.panHandlers}>
      <View style={{ flex: 1 }}>
